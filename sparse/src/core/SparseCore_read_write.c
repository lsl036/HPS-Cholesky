	...
	if (first && (strncmp (buf, "%%MatrixMarket", 14) == 0))  //读数据头
	{
		...
		*stype_new = STYPE_UNSYMMETRIC ;
	    ...
	    else if (c == 's' && c2 == 'y')
	    {
			...
			*stype_new = STYPE_SYMMETRIC_LOWER ;
	    }
	    else if (c == 'h')
	    {
			/* 埃米尔特矩阵（下三角部分） */
	    ...
	    else if (c == 's' && c2 == 'k')
	    {
			...
			*stype_new = STYPE_SKEW_SYMMETRIC ;
	    }
	    else
	    {
		/* 错误的格式 */
...
)
{
    ...
    int stype, mtype, xtype_new, stype_new ;
    ...
    if (!read_header (f, buf, &mtype, &nrow, &ncol, &nnz, &stype, &stype_new, &xtype_new) ||
	mtype != SPARSE_TRIPLET)
    {
...
)
{
    char buf [MAXLINE+1] ;
    size_t nrow, ncol, nnz ;
    int stype, mtype, stype_new, xtype_new ;
...
)
{
    ...
    if (*mtype == SPARSE_TRIPLET)
    {
		...
		Tx = T->x;
		// A->nnz = real_nnz;
		// double density = (double) real_nnz / (nrow*nrow);
		FILE *fresult_extinfo, *fresult_Edge, *fresult_Node;
		printf ("Start write graph %d's edge info\n", graph_id);
		// char *extinfo_file = (char *)"./raw/matformat_extinfo.txt";
		char *Edge_file = (char *)"./raw/adj_matformat.cites";

		// fresult_extinfo = fopen(extinfo_file ,"a+");
		fresult_Edge = fopen(Edge_file ,"a+");

		int *Degree;
		Degree = (int *)malloc(nrow * sizeof(int));
		// double *col_val;
		// col_val = (double *)malloc(ncol * sizeof(double));
		...
		Int max_deg = -1, min_deg = 9999;
		double avg_degree, max_percent, min_percent, degree_sum = 0, maxdeg_sum = 0, mindeg_sum = 0;
		for (size_t k = 0; k < nrow; k++)
		{
			max_deg = MAX(max_deg, Degree[k]);
			min_deg = MIN(min_deg, Degree[k]);
			degree_sum += Degree[k]; 
		}
		avg_degree = (double) degree_sum/ nrow;
		for (size_t k = 0; k < nrow; k++) //度最大的出现频率
		{
			if ( Degree[k]== max_deg )     
			{
				maxdeg_sum++;
			}
			else if ( Degree[k]== min_deg )     
			{
				mindeg_sum++;
			}
			// col_val[k] = col_val[k]/Degree[k];
		}
		max_percent = (double) maxdeg_sum/nrow;
		min_percent = (double) mindeg_sum/nrow;
		
		printf ("Write graph %d edge finished\n", graph_id);
